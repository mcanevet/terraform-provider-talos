...
func TestGetClientConfigurationValues_Reconstructed(t *testing.T) {
	...
	// Test extraction - this will FAIL with current implementation
	// because .As() cannot convert this plain map to clientConfiguration struct
	ca, cert, key, errMsg, ok := getClientConfigurationValues(ctx, objValue)

	if !ok {
		t.Fatalf("Expected extraction to succeed (this is the bug we're fixing), got error: %s", errMsg)
	}
