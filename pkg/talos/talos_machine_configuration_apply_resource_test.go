		...
		Steps: []resource.TestStep{
			{
				...
				// Expected drift: Ephemeral secrets regenerate on each evaluation, producing different
				// machine_configuration values. This is the documented anti-pattern (using ephemeral
				// secrets without persistence). In production, use a secret manager as documented.
